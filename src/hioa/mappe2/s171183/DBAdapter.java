...
public class DBAdapter extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String createTable = "create table " + TABLE + "(" + ID
				...
				+ " text, " + LAST_NAME + " text, " + PHONENUMBER + " text, "
				+ BIRTHDAY + " text);";
	...
	public ArrayList<Contact> selectAll() {
		...
		if (cursor.moveToFirst()) {
			while (cursor.isAfterLast() == false) {
				...
				Contact c = new Contact(id, firstname, lastname, phoneNumber,
						birthday);
				allContacts.add(c);
				cursor.moveToNext();
			}
	...
	public Contact getContact(int id) {
		open();
		db = this.getReadableDatabase();
		Cursor cursor = db.query(TABLE, new String[] { ID, FIRST_NAME,
	...
	public ArrayList<Contact> getContactsByBirthday(String birthday) {
		...
		Cursor cursor = db.query(TABLE, new String[] { FIRST_NAME, LAST_NAME,
